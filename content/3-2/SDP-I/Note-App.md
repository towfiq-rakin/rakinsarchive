...
- [ ] Block-based Markdown editor (prose-mirror / TipTap)
- [ ] `[[wiki-link]]` bidirectional note linking
- [ ] KaTeX math rendering (`$inline$` and `$$block$$`)
- [ ] Mermaid diagram rendering (fenced code blocks)
- [ ] Code blocks with syntax highlighting (Shiki / Prism)
- [ ] Inline images, embeds
- [ ] Frontmatter metadata (title, tags, slug, publishedAt)

### 3.2 Navigation — Left Sidebar

- [ ] Tree-view of all notes & blogs mirroring folder structure in DB
- [ ] Create / rename / delete / move notes and folders
- [ ] Drag-and-drop reorder
- [ ] Search (fuzzy) across all notes
- [ ] Tag / label filtering

### 3.3 Right Panel

- [ ] **Table of Contents** — auto-generated from heading hierarchy
- [ ] **Graph View** — D3.js force-directed graph of linked notes, visible in editor and on public pages

### 3.4 AI Assistant (Copilot)

- [ ] Floating toggle button (top-right, chat bubble)
- [ ] Slide-over panel with chat conversation
- [ ] Gemini Flash/Pro API backend
- [ ] Context-aware (current note content sent as system context)
- [ ] Capabilities: summarize, expand, fix grammar, explain, Q&A

_See `AI_ASSISTANT.md` for library comparison._

### 3.5 Publishing & Blogging

- [ ] Note → Blog toggle (sets `type: blog`)
- [ ] Slug-based public URLs: `/blog/[slug]` and `/notes/[slug]`
- [ ] SEO metadata (Open Graph, Twitter Card, canonical URLs)
- [ ] RSS feed at `/feed.xml`

### 3.6 Auth

- [ ] `next-auth` v5 with **Credentials** (email + password sign-up/login)
- [ ] Optional **OAuth providers**: GitHub, Google (plug-in via next-auth)
- [ ] Protected `/app/*` routes; public `/u/[username]`, `/blog/*`, `/notes/*`
- [ ] Email verification on sign-up (Resend / Nodemailer)
- [ ] Role field on User: `user` | `admin`
- [ ] Admin dashboard at `/admin` (user list, ban, promote)

### 3.7 Settings

- [ ] Theme toggle (light / dark / system)
- [ ] Custom domain CNAME support (future)
